...
func generatePDF(id, body string) error {
	t := time.Now().Unix()

	filePath := fmt.Sprintf("storage/%s-%s.html", id, strconv.FormatInt(int64(t), 10))
	pdfPath := fmt.Sprintf("storage/%s-%s.pdf", id, strconv.FormatInt(int64(t), 10))
...
func generatePDFNoHTML(id, body string) error {
	// begin := time.Now()
	t := time.Now().Unix()
	pdfPath := fmt.Sprintf("storage/%s-%s.pdf", id, strconv.FormatInt(int64(t), 10))
